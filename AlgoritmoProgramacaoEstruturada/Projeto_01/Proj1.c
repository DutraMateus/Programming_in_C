...
void RestararDadosUsuarios(char NomeCompleto[][40], char email[][30], char sexo[][10], char endereco[][50], double *altura, int vacina[], int id[], char backupNomeCompleto[][40], char backupEmail[][30], char backupSexo[][10], char backupEndereco[][50], double *backupAltura, int backupVacina[])
{

    for (int i = 0; i < numUsuarios; i++)
    {
        strcpy(NomeCompleto[numUsuarios], backupNomeCompleto[numUsuarios]);
        strcpy(email[numUsuarios], backupEmail[numUsuarios]);
        strcpy(sexo[numUsuarios], backupSexo[numUsuarios]);
        strcpy(endereco[numUsuarios], backupEndereco[numUsuarios]);
        altura[numUsuarios] = backupAltura[i];
        vacina[numUsuarios] = backupVacina[i];
    }
...
int main()
{
    ...
    do
    {
        ...
        case 6:
            ...
            break;
        case 7:
            RestararDadosUsuarios(NomeCompleto, email, sexo, endereco, altura, vacina, id, backupNomeCompleto, backupEmail, backupSexo, backupEndereco, backupAltura, backupVacina);
            break;
        default:
